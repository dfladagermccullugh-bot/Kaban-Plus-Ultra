...
- [x] Live local-Docker trial (Windows + Git Bash + Docker Engine) — found + fixed the `/setup` 404 every fresh self-host hit: `docker/.env.example` was missing `PGRST_DB_SCHEMAS`, so the installer-generated `.env` left PostgREST with an empty exposed-schema list and it 406-rejected every `Accept-Profile: public` request (i.e. every server-side supabase-js call). Added `PGRST_DB_SCHEMAS=public,storage,graphql_public` + the four `MAILER_URLPATHS_*` + `LOGFLARE_API_KEY` to `.env.example`; `install-kaban.sh` now backfills any `.env.example` key missing from an existing `.env` (secrets untouched) so upgrades self-heal; `setup-gate` now logs *why* it denies instead of a silent 404. Confirmed on the live stack: `/setup → 200`, supabase-js `HEAD → 200`. See ADR 0025.
- [x] Browser→Supabase path bridged — Caddy only listened on the public host and proxied everything to `web:3000`, so every browser-side supabase-js call (`/rest/v1`, `/auth/v1`, `/storage/v1`, `/realtime/v1`, `/functions/v1`) and the first-run magic link 404'd against Next instead of reaching Kong. `docker/Caddyfile` now reverse-proxies the five Supabase prefixes (verified against pinned `kong.yml` `v1.24.09`) to `kong:8000` via `handle` blocks; Studio's Kong catch-all is deliberately **not** proxied (admin UI stays off non-localhost hosts). `lib/env.ts:toPublicStorageUrl` generalized → `toPublicUrl` and reworked to **origin-swap** (the live operator trial proved GoTrue stamps the request host → Kong renders `http://kong`, no port, which the old prefix-match missed) so the first-run magic link is born clickable. `deploy-smoke.yml` gains a browser-equivalent `/auth/v1/health` → GoTrue assertion. Live trial: ADR-0026 routing proven on a real Docker host (`/auth/v1/health`, `/rest/v1/`, `/setup`, `/` all correct, Studio not leaked); magic-link host bug found + fixed same session. See ADR 0026.
