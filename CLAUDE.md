...
- Latest tip: `claude/docker-deployment-fixes-qjYAp` — landed **all eight ADR-0022 local-Docker deployment fixes plus the `.env.example` quoting bonus in one commit**, so a fresh `git clone → KABAN_HOST=localhost bash scripts/install-kaban.sh` is intended to run end-to-end with zero manual workarounds (incl. Windows + Git Bash). Bug #6 fix: new `apps/web/lib/env.ts:getServerSupabaseUrl()` prefers `SUPABASE_INTERNAL_URL`, wired into `admin.ts`, `server.ts`, `middleware.ts`, **and the previously-overlooked server-rendered share page `app/s/[id]/page.tsx`**. Installer streams `.env` to the JWT signer via an env var (no `-v` MSYS mangling), adds `--ignore-pull-failures` + a stale-Postgres-bind-mount guard (`--wipe` opt-in); `Dockerfile.web` dropped the pre-`FROM` platform ARGs; `apps/web/public/.gitkeep` committed; `bootstrap.sh` uses a relative `COMPOSE_FILE`; Caddy serves plain HTTP for `KABAN_HOST=localhost` via a new `KABAN_SCHEME` env. New `.github/workflows/deploy-smoke.yml` brings the full stack up and probes `/setup` for 200; `docs/SELF_HOSTING.md` gained a Troubleshooting section; non-obvious implementation choices in ADR 0023. typecheck/lint/test(34)/build all green, bundles preserved. **Still owed:** the live `clone → install → curl /setup → 200` paste — no Docker daemon in the harness, so `deploy-smoke.yml` is the CI proxy until a real-host run. No PR opened (not requested).
