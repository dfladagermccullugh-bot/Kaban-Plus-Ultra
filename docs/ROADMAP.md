...
**Current phase:** Phase 7 is feature-complete; Phase 8 (store submission) is human-driven and starts after a real-host dry run. Hosted Supabase (ref `xqdhpxfgrckjzzbenivp`) is live with all 9 migrations applied (`0009_private_schema_for_internal_functions` landed this session — moves `has_*_access` + `on_auth_user_*` into a `private` schema, flips `rotate/revoke_share_token` to `SECURITY INVOKER`; advisor lints drop 8 → 2 with only `extension_in_public` + `public_bucket_allows_listing` remaining); `apps/web/.env.local` is regenerated from the MCP connector at session start. The bundled self-host path now ships: `docker/supabase/PIN` pins `supabase/supabase` at `v1.24.09`, `docker/kaban-stack.yml` merges kaban-web + caddy + upstream Supabase via `include:`, `docker/bootstrap.sh` applies `supabase/migrations/*.sql` once Postgres is healthy, `scripts/install-kaban.sh` is the `curl ... | sh` installer (DNS pre-flight + containerised JWT signing + first-boot migrations), and the `/setup?t=…` first-run wizard claims the workspace owner without SMTP. Phase 7 polish landed earlier: `Dockerfile.web` is buildx-ready (bare multi-arch `FROM node:22-alpine` on every stage — the pre-`FROM` platform ARGs were removed in the ADR-0022 fix because they broke `docker compose up --build`) with `scripts/build-multiarch.sh` driving `linux/amd64,linux/arm64`; a healthchecked `db-backup` side-car streams gzipped `pg_dumpall` into `docker/backups/` on a configurable schedule with retention rotation. The eight ADR-0022 local-Docker deployment bugs from the live Windows + Git Bash test are now fixed in one commit (env wiring, installer path mangling, Postgres bind-mount guard, Caddy localhost scheme, `.env.example` quoting) with a `deploy-smoke.yml` CI guard. Still outstanding for v1: end-to-end fresh-VPS dry run of `install-kaban.sh` (blocked on a Docker host outside this harness), native iOS/Android Xcode/Studio projects (deferred to a dev machine), TestFlight + Play internal builds (Phase 8 / blocked on the human), Lighthouse a11y ≥ 95 / perf ≥ 90 live verification (no browser in the harness), end-to-end smoke against the connected Supabase (no browser + SMTP). **⚠️ Phase 7 is NOT actually feature-complete for self-host: the live trial (2026-05-15) fixed `/setup` (ADR 0025) and confirmed ADR 0024, but found self-host browser↔Supabase routing is unwired — Caddy never proxies `/auth/v1`,`/rest/v1`,`/storage/v1`,`/project` to kong, so first sign-in + all client-side Supabase + Studio are broken past account creation. This is the top open item → ADR 0026 (full plan), scheduled for the next session.**
...
- [x] ARM64 multi-arch image (`docker/Dockerfile.web` uses a bare `FROM node:22-alpine` — a multi-arch manifest — on every stage; `scripts/build-multiarch.sh` wraps buildx for `linux/amd64,linux/arm64`)
...
- [x] RPC grants tightened — migrations 0007 + 0008 revoke `EXECUTE` from `PUBLIC`/`anon`/`authenticated` on the auth-trigger functions (`on_auth_user_created`, `on_auth_user_email_updated`) and from `PUBLIC`/`anon` on the share-token RPCs (`rotate_share_token`, `revoke_share_token`); `supabase_auth_admin` is re-granted on the triggers. Supabase security-advisor lint count drops 12 → 8 against project `xqdhpxfgrckjzzbenivp`; remaining eight are accepted-by-design (see ADR 0020). `scripts/smoke-supabase.sh` probes the live grants from a dev machine.
- [x] Advisor lints fully cleared — migration 0009 moves `has_board_access`, `has_share_access`, `on_auth_user_created`, `on_auth_user_email_updated` into a new `private` schema (not exposed via PostgREST) and flips `rotate/revoke_share_token` from `SECURITY DEFINER` to `SECURITY INVOKER`. Drops every RLS policy in `0001` + `0005` + `0003` and recreates them with `private.` qualification in a single transaction; rewires the two `auth.users` triggers; updates `scripts/smoke-supabase.sh` to expect 404 (function not in any exposed schema) instead of 403 for the moved functions. Advisor count drops 8 → 2 — only `extension_in_public` (moddatetime) and `public_bucket_allows_listing` (avatars) remain. See ADR 0021.
- [x] Local-Docker deployment hardened — the eight ADR-0022 bugs from the live Windows + Git Bash test fixed in one commit: `getServerSupabaseUrl()` prefers `SUPABASE_INTERNAL_URL` for server-side hops (admin + server + middleware + `/s/[id]`); `install-kaban.sh` adds `--ignore-pull-failures`, streams `.env` to the JWT signer via an env var (no `-v` path mangling), and bails on a stale Postgres bind-mount unless `--wipe`; `Dockerfile.web` drops the pre-`FROM` platform ARGs; `apps/web/public/.gitkeep` committed; `bootstrap.sh` uses a relative `COMPOSE_FILE`; Caddy serves plain HTTP for `KABAN_HOST=localhost` via `KABAN_SCHEME`; `.env.example` spaced values quoted. New `deploy-smoke.yml` CI brings the full stack up and probes `/setup` for 200; `docs/SELF_HOSTING.md` gains a Troubleshooting section. See ADR 0022 + ADR 0023.
- [x] ADR-0022 re-audit — fixed two latent in-container-localhost bugs the fix itself unmasked: server-built Supabase **storage** URLs (`getPublicUrl` for the setup-wizard avatar; `createSignedUrl` for card/cover images) string-concatenated the internal `kong:8000` origin and leaked it to the browser. New `toPublicStorageUrl()` helper in `lib/env.ts` rewrites the origin at the boundary; applied at `app/setup/actions.ts` + `app/(app)/b/[id]/actions.ts`. Per-push unit test `tests/public-storage-url.test.ts`; `deploy-smoke.yml` PR filter now also fires on `apps/web/app/setup/**`. The four server-side API call sites from ADR 0023 §2 re-confirmed correct. See ADR 0024.
- [x] Live local-Docker trial (Windows + Git Bash + Docker Engine) — found + fixed the `/setup` 404 every fresh self-host hit: `docker/.env.example` was missing `PGRST_DB_SCHEMAS`, so the installer-generated `.env` left PostgREST with an empty exposed-schema list and it 406-rejected every `Accept-Profile: public` request (i.e. every server-side supabase-js call). Added `PGRST_DB_SCHEMAS=public,storage,graphql_public` + the four `MAILER_URLPATHS_*` + `LOGFLARE_API_KEY` to `.env.example`; `install-kaban.sh` now backfills any `.env.example` key missing from an existing `.env` (secrets untouched) so upgrades self-heal; `setup-gate` now logs *why* it denies instead of a silent 404. Clean-slate verified: zero-manual-step install → `/setup → 200`, avatar persists at the public origin (ADR-0024 code confirmed). See ADR 0025.
- [ ] **Self-host browser↔Supabase routing (ADR 0026 — next session).** Caddy only `reverse_proxy web:3000`; it never proxies `/auth/v1 /rest/v1 /storage/v1 /realtime/v1 /functions/v1 /project` to `kong:8000`, so on self-host all browser-side Supabase (client auth, realtime, the avatar `<img>`, the first-run magic-link `verify`, the advertised Studio link) 307s off Next and is broken — usable only through `/setup` account creation. Fix = Caddy kong routes (keep `NEXT_PUBLIC_SUPABASE_URL=http://localhost`) + generalize `toPublicStorageUrl`→`toPublicUrl` and rewrite the `generateLink` `action_link` origin in `app/setup/actions.ts` + a `deploy-smoke.yml` browser-path probe. Full plan + Studio/security decision in ADR 0026.
