...
- Active branch: each web session is assigned a fresh branch by the harness (e.g. `claude/kaban-plus-ultra-dev-<id>`). Honor whatever branch the system prompt names — never push to `main`. Previous session branches: `claude/chat-analysis-app-2fsuX` (Phases 0–1), `claude/kanban-plus-ultra-dev-VNoFa` (Phase 2), `claude/kaban-plus-ultra-dev-8Vmfo` (Phase 3), `claude/kaban-phase-3-continue-Dm3T6` (Phase 4 kickoff — realtime + presence), `claude/phase-4-realtime-channel-MkZ7T` (Phase 4 closeout — invites, share links, peer-editing, label mgmt, virtualization; merged via PRs #11 + #12), `claude/kaban-phase-4-continue-Kals8` (Phase 4 audit-writer + invite hardening + Phase 5 kickoff; merged via PRs #13 + #14), `claude/kanban-phase-5-dev-bJXbA` (Phase 5 closeout — camera plugin — + Phase 6 kickoff — Markdown ZIP export).
- Latest tip: `claude/kanban-phase-5-dev-bJXbA` at `cdd5600` (Phase 5 → 6 transition: `@capacitor/camera` wired with a web `<input type=file capture>` fallback through `apps/web/lib/camera.ts`; `/b/[id]/export` route handler + `<ExportButton>` produce a Markdown `.zip` with YAML frontmatter; `apps/mobile/README.md` native-projects checklist; `profiles.email` documented as PII in `docs/SECURITY.md`). Stacks on `main` at `f9c056c` (merge of PR #14). Unmerged — review/merge before the next session, or stack the new branch on top of it.
