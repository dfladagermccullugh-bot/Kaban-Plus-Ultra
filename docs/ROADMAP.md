...
- [x] Browser→Supabase path bridged — Caddy only listened on the public host and proxied everything to `web:3000`, so every browser-side supabase-js call (`/rest/v1`, `/auth/v1`, `/storage/v1`, `/realtime/v1`, `/functions/v1`) and the first-run magic link 404'd against Next instead of reaching Kong. `docker/Caddyfile` now reverse-proxies the five Supabase prefixes (verified against pinned `kong.yml` `v1.24.09`) to `kong:8000` via `handle` blocks; Studio's Kong catch-all is deliberately **not** proxied (admin UI stays off non-localhost hosts). `lib/env.ts:toPublicStorageUrl` generalized → `toPublicUrl` (logic unchanged) and now also rewrites the `auth.admin.generateLink` magic link off the internal origin so it's clickable. `deploy-smoke.yml` gains a browser-equivalent `/auth/v1/health` → GoTrue assertion. See ADR 0026.
