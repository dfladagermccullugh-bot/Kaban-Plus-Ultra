...
- Latest tip: `main` at `95e1146` (PR #28 merge — migration 0009 + ADR 0021 landed). After that merge, the same `claude/continue-kaban-development-obZj7` branch ran a live local-Docker deployment test on a Windows + Docker Desktop + Git Bash host. The stack does run end-to-end **only after eight discrete workarounds** — captured in ADR 0022. The blocker is bug #6: `apps/web/lib/env.ts:getSupabaseUrl()` ignores `SUPABASE_INTERNAL_URL` (which `kaban-stack.yml` already provides), so the server-side admin client fetches `http://localhost/rest/v1/profiles` from inside the `kaban-stack-web-1` container, fails, and `/setup` returns 404. Runtime env overrides don't help because Next.js inlines `NEXT_PUBLIC_*` at build time. **Next session's job:** land all eight ADR-0022 fixes as one focused commit (the operator's stated goal: `git clone → docker compose up -d → bootstrap.sh → browser` with no manual mitigations). No code was changed this session per the operator's explicit directive.
