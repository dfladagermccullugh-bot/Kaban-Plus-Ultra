...
- Active branch: each web session is assigned a fresh branch by the harness (e.g. `claude/kaban-plus-ultra-dev-<id>`). Honor whatever branch the system prompt names — never push to `main`. Previous session branches: `claude/chat-analysis-app-2fsuX` (Phases 0–1), `claude/kanban-plus-ultra-dev-VNoFa` (Phase 2), `claude/kaban-plus-ultra-dev-8Vmfo` (Phase 3), `claude/kaban-phase-3-continue-Dm3T6` (Phase 4 kickoff — realtime + presence), `claude/phase-4-realtime-channel-MkZ7T` (Phase 4 closeout — invites, share links, peer-editing, label mgmt, virtualization; merged via PRs #11 + #12), `claude/kaban-phase-4-continue-Kals8` (Phase 4 audit-writer + invite hardening + Phase 5 kickoff; merged via PRs #13 + #14), `claude/kanban-phase-5-dev-bJXbA` (Phase 5 closeout — camera plugin — + Phase 6 kickoff — Markdown ZIP export; merged via PRs #15 + #16), `claude/kaban-phase-6-continuation-P0kwC` (Phase 6a — Markdown ZIP import round-trip).
- Latest tip: `claude/kaban-phase-6-continuation-P0kwC` at `5c91150` (Phase 6a: Markdown ZIP import. Pure parser in `@kpu/core/markdown-import.ts` with 13 new tests; `importBoardFromZip` server action on `/boards` creates a new board, `mergeBoardFromZip` on `/b/[id]` merges by case-insensitive title match — appends missing rows/columns/labels, never overwrites cards; shared `<ZipDropzone>` window-level overlay with `[contenteditable]` / `.ProseMirror` guard so Tiptap drops aren't stolen; 20 MB cap; ADR 0010). Stacks on `main` at `cf7fdf4` (merge of PR #16). Unmerged — review/merge before the next session, or stack the new branch on top of it.
