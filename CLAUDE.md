...
- Previous tip merged: `main` at `d4cf4c6` (PR #29 — docs-only ADR-0022 catalogue from `claude/continue-kaban-development-obZj7`).
- Previous tip merged: `claude/docker-deployment-fixes-qjYAp` — all eight ADR-0022 local-Docker fixes + `.env.example` quoting; `getServerSupabaseUrl()` prefers `SUPABASE_INTERNAL_URL` (admin/server/middleware/`s/[id]`); installer JWT-signer env-var streaming, `--ignore-pull-failures`, stale-Postgres guard; `Dockerfile.web` dropped pre-`FROM` platform ARGs; `KABAN_SCHEME`; `deploy-smoke.yml` (merged via PRs #30/#31).
- Latest tip: `claude/kaban-plus-ultra-dev-QkBJ6` (HEAD `687f9da`, pushed; PR #31 merged it to `main` at `04d6d0a`). From a **live operator Docker trial** (Windows + Git Bash) — two landed fixes + one big documented finding:
  1. **ADR 0024** (`9e8513d`) — server-built Supabase **storage** URLs (`getPublicUrl`/`createSignedUrl`) leaked the internal `kong:8000` origin to the browser; new `apps/web/lib/env.ts:toPublicStorageUrl()` rewrites at the boundary, applied in `app/setup/actions.ts` + `app/(app)/b/[id]/actions.ts`; unit test `tests/public-storage-url.test.ts`. Live-confirmed: avatar persists as `http://localhost/storage/...`.
  2. **ADR 0025** (`687f9da`) — `docker/.env.example` was missing `PGRST_DB_SCHEMAS`, so the installer-generated `.env` left PostgREST with an empty schema list → 406 on every `Accept-Profile: public` request → **every fresh self-host `/setup` 404'd**. Added it (+ four `MAILER_URLPATHS_*` + `LOGFLARE_API_KEY`); `install-kaban.sh` now backfills missing `.env.example` keys into an existing `.env` (secrets untouched); `setup-gate.server.ts` logs *why* it denies (was a silent 404). Clean-slate live-verified: zero-manual install → `/setup → 200`. Trial diagnostics added then reverted (`5e677f0`/`12e72dc`/`a6e42ce` stay in history; `687f9da` tree clean — no force-push).
  3. **ADR 0026 — TOP OPEN ITEM, next session.** Self-host browser↔Supabase routing is unwired: Caddy only `reverse_proxy web:3000`, never proxies `/auth/v1`,`/rest/v1`,`/storage/v1`,`/realtime/v1`,`/functions/v1`,`/project` to `kong:8000`. So the first-run magic link is `http://kong/...` (NXDOMAIN), and the avatar `<img>`, client auth, realtime, Studio are broken past account creation. ADR 0026 carries the full plan. typecheck/test(39)/lint/build green at `687f9da`; git clean. No PR opened (not requested).
