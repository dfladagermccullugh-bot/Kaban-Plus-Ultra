...
- Latest tip: `main` at `af7e6cb` (PR #26 merge). Phase 7 closed; Phase 8 prep landed; this session (`claude/kaban-phase-8-continuation-enJqD`) tightened Supabase RPC grants on the connected project: migrations `0007_revoke_rpc_grants` + `0008_restore_auth_trigger_grants` revoke `EXECUTE` from `anon`/`authenticated`/`PUBLIC` on the auth-trigger functions and from `PUBLIC`/`anon` on the share-token RPCs, then re-grant `supabase_auth_admin` so signups still fire. Supabase security-advisor lints drop 12 → 8 against ref `xqdhpxfgrckjzzbenivp`; remaining eight accepted by design (ADR 0020). `scripts/smoke-supabase.sh` probes the live grants from a dev machine and auto-detects the harness's sandboxed network.
