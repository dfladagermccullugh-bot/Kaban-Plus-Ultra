...
- Active branch: each web session is assigned a fresh branch by the harness (e.g. `claude/kaban-plus-ultra-dev-<id>`). Honor whatever branch the system prompt names — never push to `main`. Previous session branches: `claude/chat-analysis-app-2fsuX` (Phases 0–1), `claude/kanban-plus-ultra-dev-VNoFa` (Phase 2), `claude/kaban-plus-ultra-dev-8Vmfo` (Phase 3), `claude/kaban-phase-3-continue-Dm3T6` (Phase 4 kickoff — realtime + presence), `claude/phase-4-realtime-channel-MkZ7T` (Phase 4 closeout — invites, share links, peer-editing, label mgmt, virtualization; merged via PRs #11 + #12), `claude/kaban-phase-4-continue-Kals8` (Phase 4 audit-writer + invite hardening + Phase 5 kickoff; merged via PRs #13 + #14), `claude/kanban-phase-5-dev-bJXbA` (Phase 5 closeout — camera plugin — + Phase 6 kickoff — Markdown ZIP export), `claude/kaban-phase-6-continuation-P0kwC` (Phase 6a — Markdown ZIP import; merged via PR #17).
- Latest tip: `claude/markdown-zip-import-7c2Oc` (Phase 6b/c/d — Framer Motion page transitions on the `(app)` segment + bottom-sheet card-editor modal on `(pointer: coarse)` with reduced-motion fallback; axe-core wired into Vitest as an a11y CI gate; `next/dynamic` Tiptap import drops `/b/[id]/c/[cardId]` from 263 → 161 kB First Load JS, and deferring the Supabase browser client at sign-in drops `/sign-in` from 153 → 116 kB). Stacks on `main` at `82bcdb0` (merge of PR #17). Unmerged — review/merge before the next session, or stack the new branch on top of it.
